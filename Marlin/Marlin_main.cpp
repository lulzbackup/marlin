...
void setup()
{
  ...
  SERIAL_ECHOLNPGM(LULZBOT_TOOLHEAD_VER);
    ...
    case 29: // G29 Detailed Z-Probe, probes the bed at 3 or more points.
        {
            ...
            if(reprobe_attempts[1] && reprobe_attempts[0] && probePointCounter == -1)
            {
              ...
              #if EXTRUDERS > 1
              active_extruder = 1;
              plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], 15.0, feedrate/60, active_extruder);
              #else
              plan_set_position(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS]);
              #endif
              st_synchronize();
              #if EXTRUDERS > 1
              active_extruder = 0;
              plan_set_position(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], 0.0);
              #endif
              disable_e0();
              disable_e1();
              disable_e2();
